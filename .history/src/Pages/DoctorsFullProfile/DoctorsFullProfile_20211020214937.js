...
const DoctorsFullProfile = () => {
    ...
    return (
        <div>
            ...
            {
                data.filter(newData=>bookId==newData.id).map(doctor=><Row key={doctor.id}> 
                <Col className="mb-5 mt-5"><Image src={doctor.photo} alt="" /></Col>
                    <Col className="doctor-full">
                        ...
                        <h6 className="doctor-qualification-title">PROFESSIONAL QUALIFICATION</h6>
                        <p className="doctor-full-qualification">{ doctor.qualification}</p>
